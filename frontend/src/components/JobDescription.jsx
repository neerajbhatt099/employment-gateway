...
import { APPLICATION_API_USER_END_POINT, JOB_API_END_POINT } from '@/utils/constant';
...
const JobDescription = () => {
    ...
    const applyJobHandler = async () => {
        try {
            const res = await axios.get(`${APPLICATION_API_USER_END_POINT}/apply/${jobId}`,
                {
                    headers: {
        ...
        const fetchSingleJobs = async () => {
            try {
                ...
                if (res.data.success) {
                    ...
                    setIsApplied(res.data.job.application.some(application => application.applicant === user?._id)) // ensure the state sync with fresh data
                }
            } catch (error) {
                console.log(error)
            }
    ...
    return (
        <div className='max-w-7xl mx-auto my-10'>
            <div className='flex items-center justify-between'>
                ...
                <Button onClick={isApplied ? null : applyJobHandler} disabled={isApplied} className={`rounded-lg ${isApplied ? 'bg-gray-600 cursor-not-allowed' : 'bg-[#7209b7] hover:bg-[#5f32ad]'}`}>{isApplied ? "Already Apllied" : "Apply Now"}</Button>
            </div>
            <div>
